...
- **Advanced Source Set Hierarchy**: Sophisticated code sharing structure with logical platform
  groupings
- **Product Flavors & Build Variants**: KMP-native `consumer × tier × buildType` flavor system — deploy your own app, a demo app, and white-label client builds from the same codebase
...
This template uses [kmp-product-flavors](https://github.com/MobileByteLabs/kmp-product-flavors) to manage multi-dimensional build variants across all KMP targets.

### Flavor dimensions

| Dimension | Values | Controls |
|-----------|--------|---------|
| `consumer` | `internal`, `demo`, `clientA`, `clientB` | Server URLs, URL override capability |
| `tier` | `advanced` (default), `basic` | Feature flags compiled in/out |
| Build type | `debug`, `staging`, `release` | Active server URL, signing, distribution target |

**24 variants total** (4 × 2 × 3). Only a subset are published to stores:

| Variant | Store | Purpose |
|---------|-------|---------|
| `internalAdvancedRelease` | Your App Store / Play Store | Your production app |
| `demoAdvancedRelease` | Your App Store / Play Store | Prospects — includes runtime server URL override |
| `clientABasicRelease` | Client A's store account | White-label basic tier |
| `clientAAdvancedRelease` | Client A's store account | White-label advanced tier |

### Building a specific variant

```bash
# List all 24 variants
./gradlew listFlavors
...
- [Build Variants & CI/CD](docs/BUILD_VARIANTS.md) — pipeline, deployment targets, app IDs
- [Integration Guide](docs/claude/product-flavors-integration.md) — how to wire this into any KMP project
