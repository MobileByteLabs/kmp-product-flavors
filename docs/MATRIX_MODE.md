...
| `variantCacheNamespacing` opt-in stub (v2.3) | Reserved DSL surface for variant-scoped Gradle build-cache namespacing. **Ships as a no-op pending real cache-miss telemetry from 8+ variant modules**; exposed now so consumers can forward-declare opt-in without a future DSL break. See `internal/VariantBuildCacheKeyConfigurator.kt` KDoc for the path-(a) vs path-(b) split. | v2.3 Phase 2 (stub) |
