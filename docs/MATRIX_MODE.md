...
| `variantCacheNamespacing` opt-in (v2.4) | Variant-scoped Gradle build-cache namespacing. Injects `kmpFlavorVariant` as an `@Input` property on every `compileKotlin*` task in matrix mode, partitioning the cache key space per variant. Active-variant tasks namespace as `active`. Prerequisite: `buildMatrix=true`. Default `false` until real-world telemetry justifies the flip; see `internal/VariantBuildCacheKeyConfigurator.kt` KDoc for the impl details. | v2.4 Phase 2 path-(b) |
| Variant-conditional dep excludes (v2.4) | DSL `kmpFlavors.variants.matching { … }.configureEach { dependencies { exclude(group, module) } }`. Excludes scoped per-variant; each `KotlinCompilation`'s compile + runtime classpath configurations get the exclude rule applied. KMPF-V22 WARNING when both group + module are empty. See [`docs/VARIANT_DEPENDENCY_EXCLUDES.md`](VARIANT_DEPENDENCY_EXCLUDES.md). | v2.4 Phase 5 |
...
| `org.jetbrains.compose` (Compose Multiplatform) | ✅ Compatible (v2.2 adds Option B switcher + IDE plugin) | Per-variant compilations honor Compose's own source-set hierarchy. **Per-variant `composeResources/` work end-to-end** via the source-set convention (v2.1+). **Hot-reload is still active-variant only**; per-variant hot-reload (true Option A) is deferred to v2.3+. v2.2 ships Option B: the `./gradlew listActiveVariant` CLI helper + the [`kmp-product-flavors-ide-plugin`](https://plugins.jetbrains.com/plugin/31779-kmp-product-flavors) IntelliJ plugin (live on JetBrains Marketplace; variant-switcher dropdown + Gradle-tool-window grouping) for a workable per-variant hot-reload UX (switch via `-PkmpFlavor=…`, then re-run `composeApp:run`). |
