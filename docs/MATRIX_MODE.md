...
| Variant-conditional dep excludes (v2.3 Phase 3 — docs-only) | DSL `kmpFlavors.variants.matching { … }.configureEach { dependencies { exclude(group, module) } }`. Survey-gated to ≥5 consumer requests; meanwhile docs at [`docs/VARIANT_DEPENDENCY_EXCLUDES.md`](VARIANT_DEPENDENCY_EXCLUDES.md) include the manual workaround. | v2.3 Phase 3 (docs only) |
