...
generated_class: org.openmf.kmptemplate.BuildKonfig   # configurable via buildConfigPackage + buildConfigClassName
...
|---|---|---|
| **BuildKonfig codegen** | Generates a Kotlin object with compile-time constants for every flavor+buildType combination | `buildConfigPackage`, `buildConfigClassName`, `buildConfigField(...)` |
    ...
    override fun apply(target: Project) {
        with(target) {
            ...
            extensions.configure<KmpFlavorExtension> {
                // Generated object: org.openmf.kmptemplate.BuildKonfig
...
> Apply to every module that needs `BuildKonfig` constants or flavor-specific source sets. Typically `cmp-shared` and any feature module that branches on flavor.

---

### Step 8 — AppVariant accessor: `cmp-shared/src/commonMain/kotlin/cmp/shared/flavor/AppVariant.kt`

Create a typed wrapper so call sites never import `BuildKonfig` directly:
...
object AppVariant {
    ...
    val isInternal: Boolean  get() = BuildKonfig.IS_INTERNAL
    val isDemo: Boolean      get() = BuildKonfig.IS_DEMO
    val isClientA: Boolean   get() = BuildKonfig.IS_CLIENT_A
    val isClientB: Boolean   get() = BuildKonfig.IS_CLIENT_B

    // Tier flavor booleans
    val isAdvanced: Boolean  get() = BuildKonfig.IS_ADVANCED
    val isBasic: Boolean     get() = BuildKonfig.IS_BASIC

    // Consumer custom fields
    val clientId: String          get() = BuildKonfig.CLIENT_ID
    val apiUrlDebug: String        get() = BuildKonfig.API_URL_DEBUG
    val apiUrlStaging: String      get() = BuildKonfig.API_URL_STAGING
    val apiUrlRelease: String      get() = BuildKonfig.API_URL_RELEASE
    val allowUrlOverride: Boolean  get() = BuildKonfig.ALLOW_URL_OVERRIDE

    // Tier custom fields
    val clientTier: String              get() = BuildKonfig.CLIENT_TIER
    val featureAnalytics: Boolean       get() = BuildKonfig.FEATURE_ANALYTICS
    val featureReports: Boolean         get() = BuildKonfig.FEATURE_REPORTS
    val featureBulkOperations: Boolean  get() = BuildKonfig.FEATURE_BULK_OPERATIONS
    ...
    val allowEnvSwitch: Boolean   get() = BuildKonfig.ALLOW_ENV_SWITCH
...
actual object ContentRepository {
    ...
    actual fun getBaseUrl(): String = BuildKonfig.API_URL_RELEASE
...
## GENERATED BuildKonfig API

After a successful Gradle sync, the plugin generates `org.openmf.kmptemplate.BuildKonfig` in `commonMain`. All fields are `const val`.
...
```
internalAdvanced + debug   → BuildKonfig.API_URL_DEBUG    = "https://dev.yourdomain.com"
internalAdvanced + staging → BuildKonfig.API_URL_STAGING  = "https://staging.yourdomain.com"
internalAdvanced + release → BuildKonfig.API_URL_RELEASE  = "https://api.yourdomain.com"
```
...
# 3. Confirm BuildKonfig is generated
./gradlew :cmp-shared:kspCommonMainKotlinMetadata
# Generated file location (relative to cmp-shared):
# build/generated/kmp-flavors/commonMain/kotlin/org/openmf/kmptemplate/BuildKonfig.kt
...
| `BuildKonfig` not found in commonMain | Convention plugin not applied to the module | Add `alias(libs.plugins.kmp.flavors.convention)` to module's `build.gradle.kts` (Step 7) |
...
These properties are declared on `BuildKonfig` and surfaced by the `printFlavorProperties` Gradle task. Consumers wire the resolved suffix into `Window(title = ...)` (desktop) and `<title>` (web).
...
The property is read by `KmpFlavorPlugin` during `afterEvaluate` and routes which variant's `BuildKonfig` constants are baked into the build. Useful for CI matrices that build many variants from the same checkout. Default behaviour (no `-PkmpFlavor`): the variant formed by each dimension's `isDefault.set(true)` flavor.
