...
- The canonical claim winner in [`samples/kmp-project-template`](https://github.com/openMF/kmp-project-template) is `cmp-navigation` (deterministic across local + CI). This is what THIS repo's `.github/workflows/pr-check.yml` validates.
...
# The pr-check.yml step validates the file path against kmp-project-template
grep -E 'cmp-navigation/build/generated/kmpFlavors/commonMain/kotlin' \
