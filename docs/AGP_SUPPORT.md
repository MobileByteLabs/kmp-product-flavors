...
**Floor: AGP 9.4.0.** This library only supports Android Gradle Plugin **9.4.0 or higher**. Older AGP versions are explicitly **not supported** and **will not** be tested in CI.

## Rationale

AGP 9.x is the active stable line. AGP 9.4.0 ships:
...
Pinning to 9.4.0+ as the single supported floor lets the library:
...
- Bump consumer `gradle/libs.versions.toml`:
  ```toml
  agp = "9.4.0"  # or any newer 9.x stable patch
  ```
- Bump Gradle wrapper to **9.6.0 or higher** (AGP 9.4.0 requires Gradle 9.6.0 minimum)
