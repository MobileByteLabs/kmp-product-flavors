...
- **Line-coverage gate ramped 25 → 40** — affects contributors only. The roadmap continues toward 100% across v2.7.x point releases. See [`COVERAGE_GUIDE.md`](COVERAGE_GUIDE.md) + [`COVERAGE_DEEP_DIVE.md`](COVERAGE_DEEP_DIVE.md) (new) for the contributor playbook.
...
- **5 new direct-test classes** — `NetworkConfigSpecTest`, `BuildKonfigDslTest`, `DiDslTest`, `AnalyticsTagsConfigTest`, `FlavorVariantTest`. These document the consumer-facing DSL contracts at unit-test level.
...
- **Coverage gate** ramped from floor 25 to floor 30; PRs that regress local coverage fail CI
